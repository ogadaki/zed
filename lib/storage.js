...
var importPatch = function (patch) {
    ...
    _.each(patch.blocks, function (block) {
        if (block.tagName === 'script') {
            ...
            // output.
            editor.addBlock('html', block.tagName, block.value, 1, 1, block.top, block.left);
        } else {
            // TODO for now we only create block with exactly one input and one
            // output.
            editor.addBlock('html', block.innerHTML, '', 1, 1, block.top, block.left);
        }
...
storage.savePatch = function (name) {
...
storage.loadPatch = function (name) {
    if (name === undefined) {
