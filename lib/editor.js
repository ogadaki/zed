...
editor.addBlock = function (type) {
    ...
    editor.createBlockElement.apply(null, args);
};
...
editor.moveBy = function (leftOffset, topOffset) {
    var current = editor.getCurrentBlock();
    var top = Number(current.style.top.slice(0, -2));
    var left = Number(current.style.left.slice(0, -2));
    current.style.top = (top + Number(topOffset)) + 'px';
    current.style.left = (left + Number(leftOffset)) + 'px';
    current.redraw();
};
